...
export default {
  ...
  computed: {
    genres() {
      return [...new Set(this.videos.map(v => v.genre))].sort()
    },
    filteredVideos() {
      if (this.selectedGenre === 'all') return this.videos
      return this.videos.filter(video => video.genre === this.selectedGenre)
    }
  },
  methods: {
    filterByGenre(genre) {
